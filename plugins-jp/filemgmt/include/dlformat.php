...
if (basename($_SERVER['PHP_SELF']) == "dlformat.php") {
    exit();
}

$path = $mytree->getPathFromId($cid, "title");
$path = substr($path, 1);
$path = str_replace("/"," <img src='" .$_CONF['site_url'] ."/filemgmt/images/arrow.gif' border='0' alt=''> ",$path);

$p->set_var('LANG_CATEGORY',_MD_CATEGORYC);
$p->set_var('category_path',$path);

if ($submitter > 1) {
    $submitter_name = "<a href='" . $_CONF['site_url']
                    . "/users.php?mode=profile&amp;uid=" . $submitter . "'>"
                    . $submitter_name . '</a>';
}
...
$p->set_var('visit_url', COM_buildUrl($_CONF['site_url']
                                      . '/filemgmt/visit.php?lid=' . $lid));

$p->set_var('download_title', _MD_CLICK2DL . urldecode($url));
$p->set_var('url',$url);
if (! empty($_REQUEST['query'])) {
    $p->set_var('file_description',
                COM_highlightQuery($description, $_REQUEST['query']));
} else {
...
if ($comments) {
    ...
    if ($commentCount > 0) {
        ...
        $comment_link = '<a href="' . COM_buildUrl($_CONF['site_url'] .'/filemgmt/index.php?id=' . $lid) . '#comments" title="'.$recentPostMessage.'">' .$commentCount.'&nbsp;' .$LANG01[3]. '</a>';
    } else {
...
$file_link = COM_buildUrl($_CONF['site_url'] . '/filemgmt/index.php?id=' . $lid);
$p->set_var('file_link', COM_createLink(_MD_FILELINK, $file_link));
