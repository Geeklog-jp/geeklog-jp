...
$LANG_AUTO = array(
    ...
    'access_denied_msg' => '自動タグプラグインの管理者用ページに不正にアクセスしています。このページへの不正なアクセスは記録しています。',
    'deny_msg' => 'このページのアクセスを拒否しました。ページは移動や削除されているか、ページにアクセス権がないのかもしれません。',

    'php_msg_enabled' => 'これをチェックすると、<phpautotags_タグ名>という名前の関数を呼び出して、タグを置換します。PHPにチェックを入れて<b>置換文字列</b>を指定すると、PHPのコードとして評価します。コード内では、$p1に第1引数、$p2に第2引数、$tagstrにタグ全体を代入します。自動タグと置換する文字列を返すにはreturn文を使います。',
    ...
    'duplicate_tag' => '選択したタグは既に使用しています。他のタグを選ぶか、使用中のタグを編集してください。',
    ...
    'replace_explain' => '自動タグの記述形式は <b>[tag:parameter1 parameter2]</b> です。<br' . XHTML . '>置換文字列フィールドにはHTMLを記述できます。<br' . XHTML . '>置換文字列フィールドの文字列中に <b>#1</b> や <b>#2</b> を記述することにより、<b>parameter1</b> や <b>parameter2</b> を含めることができます。</p>'
                        .'<p>自動タグは一般にリンクを作成するために使用します。<br' . XHTML . '>タグ <b>[tag:foo This is a link]</b> が、置換文字列フィールドの文字列<br' . XHTML . '> <b>&lt;a href="http://path.to.somewhere/#1"&gt;#2&lt;/a&gt;</b> <br' . XHTML . '>に関連付けられているとき、そのタグは文字列<br' . XHTML . '> <b>&lt;a href="http://path.to.somewhere/foo"&gt;This is a link&lt;/a&gt;</b><br' . XHTML . '>に置換します。</p>'
