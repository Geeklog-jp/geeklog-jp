...
function getDeleteImageLink($mode, $A, $token)
{
    ...
    $img = '<img src="' . $_CONF['site_url']
        . '/calendarjp/images/delete_event.' . $_IMAGE_TYPE
        . '" alt="' . $LANG_CALJP_2[30] . '" title="'
        . $LANG_CALJP_2[30] . '"' . XHTML . '>';

    if ($mode == 'personal') {
        if (SEC_hasAccess($A['owner_id'], $A['group_id'], $A['perm_owner'],
                $A['perm_group'], $A['perm_members'], $A['perm_anon']) > 0) {
            $retval = COM_createLink($img, $_CONF['site_url']
                    . '/calendarjp/event.php?action=deleteevent&amp;eid='
                    . $A['eid'] . '&amp;' . CSRF_TOKEN . '=' . $token);
        }
    } else if (SEC_hasRights('calendarjp.edit')) {
        if (SEC_hasAccess($A['owner_id'], $A['group_id'], $A['perm_owner'],
                $A['perm_group'], $A['perm_members'], $A['perm_anon']) == 3) {
            $retval = COM_createLink($img, $_CONF['site_admin_url']
                    . '/plugins/calendarjp/index.php?mode=' . $LANG_CALJP_ADMIN[22]
...
function getSmallCalendar ($m, $y, $mode = '')
{
    ...
    $retval .= '<table class="smallcal">' . LB
            ...
            . COM_createLink($mycal->getMonthName ($m), $_CONF['site_url']
                . '/calendarjp/index.php?month=' . $m . '&amp;year=' . $y . $mode)
...
if ((($view == 'day') || ($view == 'week')) &&
        (($mode == 'personal') || SEC_hasRights('calendarjp.edit'))) {
...
case 'day':
    ...
    if ($mode == 'personal') {
        ...
        $cal_templates->set_var('calendar_toggle', '[&nbsp;'
            . COM_createLink($LANG_CALJP_2[11], $_CONF['site_url']
                . "/calendarjp/index.php?view=day&amp;month=$month&amp;day=$day&amp;year=$year") . '&nbsp;]'
        );
    } else {
        $cal_templates->set_var('calendar_title', $_CONF['site_name'] . ' ' . $LANG_CALJP_2[29]);
        if (!COM_isAnonUser() AND $_CAJP_CONF['personalcalendars'] == 1) {
            $cal_templates->set_var('calendar_toggle', '[&nbsp;'
                . COM_createLink($LANG_CALJP_2[12], $_CONF['site_url']
                    . "/calendarjp/index.php?mode=personal&amp;view=day&amp;month=$month&amp;day=$day&amp;year=$year") . '&nbsp;]'
            );
        } else {
            $cal_templates->set_var('calendar_toggle', '');
        }
    ...
    if ($mode == 'personal') {
        $calsql = "SELECT eid,title,datestart,dateend,timestart,timeend,allday,owner_id,group_id,perm_owner,perm_group,perm_members,perm_anon FROM {$_TABLES['personal_eventsjp']} "
                . "WHERE (uid = {$_USER['uid']}) "
        ...
        $calsql = "SELECT eid,title,datestart,dateend,timestart,timeend,allday,owner_id,group_id,perm_owner,perm_group,perm_members,perm_anon FROM {$_TABLES['eventsjp']} WHERE ((allday=1 "
    ...
    if ($alldaycount > 0) {
        for ($i = 1; $i <= $alldaycount; $i++) {
            ...
            $cal_templates->set_var('event_time', $LANG_CALJP_2[26]);
...
case 'week':
    ...
    if ($mode == 'personal') {
        ...
        $cal_templates->set_var('calendar_toggle', '[ '
            . COM_createLink($LANG_CALJP_2[11], $_CONF['site_url']
                . "/calendarjp/index.php?view=week&amp;month=$month&amp;day=$day&amp;year=$year") . ' ]'
        );
    } else {
        $cal_templates->set_var('calendar_title', $_CONF['site_name'] . ' ' . $LANG_CALJP_2[29]);
        if (!COM_isAnonUser() AND $_CAJP_CONF['personalcalendars'] == 1) {
            $cal_templates->set_var('calendar_toggle', '[ '
                . COM_createLink($LANG_CALJP_2[12], $_CONF['site_url']
                    . "/calendarjp/index.php?mode=personal&amp;view=week&amp;month=$month&amp;day=$day&amp;year=$year") . ' ]'
            );
        } else {
            $cal_templates->set_var('calendar_toggle', '');
        }
    ...
    for ($i = 1; $i <= 7; $i++) {
        ...
        if ($mode == 'personal') {
            ...
            $add_str =  $LANG_CALJP_2[42];
        }

        $cal_templates->set_var ('langlink_addevent' . $i,
            COM_createLink($add_str, $_CONF['site_url'] . '/submit.php?type=calendarjp&amp;'
            . addMode ($mode) . "day=$daynum&amp;month=$monthnum&amp;year=$yearnum")
        );
        if ($mode == 'personal') {
            $calsql = "SELECT eid,title,datestart,dateend,timestart,timeend,allday,owner_id,group_id,perm_owner,perm_group,perm_members,perm_anon FROM {$_TABLES['personal_eventsjp']} WHERE (uid = {$_USER['uid']}) AND ((allday=1 AND datestart = \"$yearnum-$monthnum-$daynum\") OR (datestart >= \"$yearnum-$monthnum-$daynum 00:00:00\" AND datestart <= \"$yearnum-$monthnum-$daynum 23:59:59\") OR (dateend >= \"$yearnum-$monthnum-$daynum 00:00:00\" AND dateend <= \"$yearnum-$monthnum-$daynum 23:59:59\") OR (\"$yearnum-$monthnum-$daynum\" BETWEEN datestart AND dateend)) ORDER BY datestart,timestart";
        } else {
            $calsql = "SELECT eid,title,datestart,dateend,timestart,timeend,allday,owner_id,group_id,perm_owner,perm_group,perm_members,perm_anon FROM {$_TABLES['eventsjp']} WHERE ((allday=1 AND datestart = \"$yearnum-$monthnum-$daynum\") OR (datestart >= \"$yearnum-$monthnum-$daynum 00:00:00\" AND datestart <= \"$yearnum-$monthnum-$daynum 23:59:59\") OR (dateend >= \"$yearnum-$monthnum-$daynum 00:00:00\" AND dateend <= \"$yearnum-$monthnum-$daynum 23:59:59\") OR (\"$yearnum-$monthnum-$daynum\" BETWEEN datestart AND dateend))" . COM_getPermSql ('AND') . " ORDER BY datestart,timestart";
        }
        $result = DB_query($calsql);
        $nrows = DB_numRows($result);
        for ($j = 1; $j <= $nrows; $j++) {
            $A = DB_fetchArray($result);
            if ($A['allday'] == 1) {
                $cal_templates->set_var('event_starttime', $LANG_CALJP_2[26]);
                ...
                if (($startmonth == $monthnum && $daynum > $startday) OR ($startmonth <> $monthnum)) {
                    ...
                    $starttime = date($tformat, $startstamp);
                }
                if (($endmonth == $monthnum && $daynum < $endday) OR ($endmonth <> $monthnum)) {
                    $endtime = date('n/j ' . $tformat, $endstamp);
                } else {
                    $endtime = date($tformat, $endstamp);
                }
            ...
            $cal_templates->set_var ('event_title_and_link',
                COM_createLink(stripslashes($A['title']), $_CONF['site_url']
                    . '/calendarjp/event.php?' . addMode ($mode)
                . 'eid=' . $A['eid'])
            );
...
if ($mode == 'personal') {
    $cal_templates->set_var ('calendar_title',
                             $LANG_CALJP_2[28] . ' ' . COM_getDisplayName());
} else {
    $cal_templates->set_var ('calendar_title',
                             $_CONF['site_name'] . ' ' . $LANG_CALJP_2[29]);
}
        ...
        if (!empty($curday)) {
            ...
            $cal_templates->set_var ('cal_day_anchortags',
                COM_createLink($curday->daynumber, $_CONF['site_url']
                    . '/calendarjp/index.php?view=day&amp;' . addMode ($mode)
                    ...
                    if ($results['title']) {
                        ...
                        $entries .=
                            COM_createLink(
                                ...
                                $_CONF['site_url'] . '/calendarjp/event.php?' . addMode ($mode)
                                . 'eid=' . $results['eid'],
...
if ($mode == 'personal') {
    $cal_templates->set_var('lang_mastercal', $LANG_CALJP_2[25] . $LANG_CALJP_2[11]);
    $cal_templates->parse('master_calendar_option','mastercal',true);
} else {
    if (!COM_isAnonUser() && ($_CAJP_CONF['personalcalendars'] == 1)) {
        $cal_templates->set_var('lang_mycalendar', $LANG_CALJP_2[12]);
...
if ($mode == 'personal') {
    $cal_templates->set_var('lang_addevent', $LANG_CALJP_2[8]);
    $cal_templates->set_var('addevent_formurl', '/calendarjp/index.php');
} else {
    $cal_templates->set_var('lang_addevent', $LANG_CALJP_2[42]);
    $cal_templates->set_var('addevent_formurl', '/submit.php?type=calendarjp');
}
