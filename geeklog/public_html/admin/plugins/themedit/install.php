...
$T->set_var('install_header', $LANG_THM['install_header']);
...
if ($action == 'install') {
    if (plugin_install_themedit()) {
        ...
        $T->set_var('installmsg1', $LANG_THM['install_failed']);
    }
} else if ($action == "uninstall") {
   plugin_uninstall_themedit('installed');
   $T->set_var('installmsg1', $LANG_THM['uninstall_msg']);
}

if (DB_count($_TABLES['plugins'], 'pi_name', 'themedit') == 0) {
    $T->set_var('installmsg2', $LANG_THM['uninstalled']);
    $T->set_var('readme', $LANG_THM['readme']);
    $T->set_var('site_admin_url', $_CONF['site_admin_url']);
    $T->set_var('installdoc', $LANG_THM['installdoc']);
    $T->set_var('installdoc_ja', $LANG_THM['installdoc_ja']);
	$T->set_var('btnmsg', $LANG_THM['install']);
	$T->set_var('action','install');
} else {
    $T->set_var('installmsg2', $LANG_THM['installed']);
	$T->set_var('btnmsg', $LANG_THM['uninstall']);
