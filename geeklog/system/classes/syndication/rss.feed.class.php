...
//    	$timezones = array('en_GB','en_US');
    	$timezones = array('C', 'en_GB','en_US'); // Geeklog Japanese Hiroron changed 2007.3.30
    ...
    function _formatArticle( $article )
    {
      ...
      if( array_key_exists( 'topic', $article ) )
      {
        ...
        if( is_array( $topic ))
        {
          ...
          {
            $xml .= '<category>'.$this->_safeXML( $subject )."</category>\n";
          }
        }
        else
        {
          $xml .= '<category>'.$this->_safeXML( $topic )."</category>\n";
        }
      }
      if( array_key_exists( 'summary', $article ) )
      {
