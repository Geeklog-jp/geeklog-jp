...
//@@@@@ 20071212 本名表示設定問題(記事の投稿者名)対応 (STORY_renderArticle)
...
function STORY_renderArticle( $A, $index='', $storytpl='storytext.thtml', $query='' )
{
   ...
   // Replace any plugin autolink tags
    ...
    if( $_CONF['contributedbyline'] == 1 )
    {
        ...
        //}
        $article->set_var( 'contributedby_user', COM_getDisplayName($A['uid']) );
        //@@@@@20071212(20071016)<----
            ...
            $A['show_topic_icon'] == 1 )
    {
        if( !empty( $A['imageurl'] ))
        {
            ...
            $topicimage = '<img style="float:' . $_CONF['article_image_align'] // align->style 2007.8.21
...
function STORY_replace_images($sid, $intro, $body)
{
    ...
    for ($i = 1; $i <= $nrows; $i++) {
        ...
        $left = $lLinkPrefix . '<img ' . $sizeattributes . 'style="float:left" src="' . $imgSrc . '" alt="">' . $lLinkSuffix; // align->style 2007.8.21
        $right = $lLinkPrefix . '<img ' . $sizeattributes . 'style="float:right" src="' . $imgSrc . '" alt="">' . $lLinkSuffix; // align->style 2007.8.21
        ...
        if (($_CONF['allow_user_scaling'] == 1) and
            ...
            if (file_exists ($lFilename_large_complete)) {
                    ...
                    $left = '<img ' . $sizeattributes . 'style="float:left" src="' . $lFilename_large_URL . '" alt="">'; // align->style 2007.8.21
                    $right = '<img ' . $sizeattributes . 'style="float:right" src="' . $lFilename_large_URL . '" alt="">'; // align->style 2007.8.21
                }
    ...
    for ($i = 1; $i <= $nrows; $i++) {
        ...
        if ($icount == 0) {
            ...
            if (count($errors) == 0) {
                ...
                $intro = str_replace($left, $lLinkPrefix . '<img ' . $sizeattributes . 'style="float:left" src="' . $imgSrc . '" alt="">' . $lLinkSuffix, $intro); // align->style 2007.8.21
                $body = str_replace($left, $lLinkPrefix . '<img ' . $sizeattributes . 'style="float:left" src="' . $imgSrc . '" alt="">' . $lLinkSuffix, $body); // align->style 2007.8.21
                $intro = str_replace($right, $lLinkPrefix . '<img ' . $sizeattributes . 'style="float:right" src="' . $imgSrc . '" alt="">' . $lLinkSuffix, $intro); // align->style 2007.8.21
                $body = str_replace($right, $lLinkPrefix . '<img ' . $sizeattributes . 'style="float:right" src="' . $imgSrc . '" alt="">' . $lLinkSuffix, $body); // align->style 2007.8.21
                ...
                if (($_CONF['allow_user_scaling'] == 1) and
                    ...
                    $intro = str_replace($unscaledleft, '<img ' . $sizeattributes . 'style="float:left" src="' . $imgSrc . '" alt="">', $intro); // align->style 2007.8.21
                    $body = str_replace($unscaledleft, '<img ' . $sizeattributes . 'style="float:left" src="' . $imgSrc . '" alt="">', $body); // align->style 2007.8.21
                    $intro = str_replace($unscaledright, '<img ' . $sizeattributes . 'style="float:right" src="' . $imgSrc . '" alt="">', $intro); // align->style 2007.8.21
                    $body = str_replace($unscaledright, '<img ' . $sizeattributes . 'style="float:right" src="' . $imgSrc . '" alt="">', $body); // align->style 2007.8.21
                }
