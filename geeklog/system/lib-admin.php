            ...
            $query_arr, $menu_arr, $defsort_arr, $filter = '', $extra = '', $options = '')
{
    ...
    // 投稿管理で承認/削除の問題。ブロック管理の先頭の有効/無効の問題 対応 2007/05/01 hiro {
    $form_start = '';
    if (!empty ($text_arr['form_start'])) {
        $form_start = $text_arr['form_start'];
    }
    $form_end = '';
    if (!empty ($text_arr['form_end'])) {
        $form_end = $text_arr['form_end'];
    }
    // } 投稿管理で承認/削除の問題。ブロック管理の先頭の有効/無効の問題 対応 2007/05/01 hiro
...
//  Geeklog Japanese Ivy changed by kinoshita  管理画面テンプレートがなければprofessionalテーマに 2007/07/23 <----
    ...
    $admin_templates->set_var('lang_delconfirm', $LANG01[125]);
    // 投稿管理で承認/削除の問題。ブロック管理の先頭の有効/無効の問題 対応 2007/05/01 hiro {
    $admin_templates->set_var('form_start', $form_start);
    $admin_templates->set_var('form_end', $form_end);
    // } 投稿管理で承認/削除の問題。ブロック管理の先頭の有効/無効の問題 対応 2007/05/01 hiro
