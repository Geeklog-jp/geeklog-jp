...
$LANG_PDF = array(
    1 => 'PDF生成ができません。',
    2 => 'ドキュメントは変換されませんでした。受け取ったドキュメントを処理できませんでした。XHTML標準によって書かれたHTML形式のドキュメントだけを用意したことを確認してください。複雑すぎるHTMLのドキュメントは正しく変換されなかったり，まったく変換されい場合があります。ドキュメントの変換結果は0バイトのサイズだったので，削除しました。ドキュメントが変換可能だと確信できる場合は，もう一度実行してみてください。',
    3 => 'PDFの生成で不明なエラーが発生しました。',
    4 => 'ページのデータが与えられていないか，以下のアドホックなPDF生成ツールを使用してください。このページがエラーだと思われる場合は，システムの管理者に連絡してください。もしくは以下のフォームから，アドホックな方式によるPDFの生成を行ってみてください。',
    5 => 'ドキュメントを読み込み中です。',
    6 => 'ドキュメントを読み込むまでしばらくお待ちください。',
    7 => '下のボタンでマウスの右ボタンをクリックしたメニューから，「対象を保存……」または「リンクの場所を保存……」を選択してドキュメントのコピーを保存してください。',
    8 => '設定ファイルに指定されたHTMLDocバイナリのパスが誤っているか，システムが実行できません。この問題が解決しない場合は，サイトの管理者に連絡を取ってください。',
    9 => 'PDF生成',
    10 => 'これはアドホックなPDF生成ツールです。URLで指定した内容をPDFに変換しようとします。この機能では適切に変換されないWebページもあることに注意してください。これはHTMLDoc PDF生成ツールの制限なので，このサイトの管理者に報告しないでください。',
    11 => 'URL',
    12 => 'PDFを生成！',
    13 => 'このサーバのPHP設定はURLをfopen()コマンドに使用することを許可していません。システム管理者はphp.iniを編集して，allow_url_fopenをOnにしてください。',
    14 => 'PDFが存在しない，あるいは不正なファイルアクセスをしようとしました。'
);
