<?php
	...
	*/
	if(!defined('KSESF_CLASS_PHP4'))
	{
		define('KSESF_CLASS_PHP4', true);
	...
	*	+ Integrated code from kses 0.2.2 into class.
	*
	*	@author     Richard R. V疽quez, Jr. (Original procedural code by Ulf H舐nhammar)
	...
	*	@copyright  Richard R. V疽quez, Jr. 2003-2005
	...
	*/
		class ksesf4
		{
			...
			function AddProtocols()
			{
				...
				if($c_args != 1)
				{
					trigger_error("ksesf4::AddProtocols() did not receive an argument.", E_USER_WARNING);
					return false;
				}
				...
				{
					trigger_error("ksesf4::AddProtocols() did not receive a string or an array.", E_USER_WARNING);
					return false;
				}
			}
			...
			function Protocols()
			{
				...
				if($c_args != 1)
				{
					trigger_error("ksesf4::Protocols() did not receive an argument.", E_USER_WARNING);
					return false;
				}
			...
			function AddProtocol($protocol = "")
			{
				...
				{
					trigger_error("ksesf4::AddProtocol() requires a string.", E_USER_WARNING);
					return false;
				}
				...
				if($protocol == "")
				{
					trigger_error("ksesf4::AddProtocol() tried to add an empty/NULL protocol.", E_USER_WARNING);
					return false;
				}
			...
			function SetProtocols()
			{
				...
				if($c_args != 1)
				{
					trigger_error("ksesf4::SetProtocols() did not receive an argument.", E_USER_WARNING);
					return false;
				}
				...
				{
					trigger_error("ksesf4::SetProtocols() did not receive a string or an array.", E_USER_WARNING);
					return false;
				}
			}
			...
			function AddHTML($tag = "", $attribs = array())
			{
				if(!is_string($tag))
				{
					trigger_error("ksesf4::AddHTML() requires the tag to be a string", E_USER_WARNING);
					return false;
				}
				...
				if($tag == "")
				{
					trigger_error("ksesf4::AddHTML() tried to add an empty/NULL tag", E_USER_WARNING);
					return false;
				}

				if(!is_array($attribs))
				{
					trigger_error("ksesf4::AddHTML() requires an array (even an empty one) of attributes for '$tag'", E_USER_WARNING);
					return false;
				}
			...
			function RemoveProtocol($protocol = "")
			{
				if(!is_string($protocol))
				{
					trigger_error("ksesf4::RemoveProtocol() requires a string.", E_USER_WARNING);
					return false;
				}
				...
				if($protocol == "")
				{
					trigger_error("ksesf4::RemoveProtocol() tried to remove an empty/NULL protocol.", E_USER_WARNING);
					return false;
				}
			...
			function RemoveProtocols()
			{
				...
				{
					trigger_error("ksesf4::RemoveProtocols() did not receive a string or an array.", E_USER_WARNING);
					return false;
				}
			}
